<%-- 
    Document   : timer
    Created on : 25-Apr-2021, 22:23:48
    Author     : Jesus Larez
--%>
...
<%@page import="entities.ProgrammableTimerRemote"%>
...
<body>
    ...
    <form action="FrontController" >
        <label for="TimerText">Insert Timer Message:</label><br>
        <input type="text" name="TimerText"><br>
        <label for="TimerDuration">Insert Timer Duration in Milliseconds</label><br>
        <input type="number" name="TimerDuration"><br>
        <input type="hidden" name="command" value="CmdSetTimer">
        <input type="submit" class="btn btn-info" value="Set Timer"> 
    </form>
    <%
        String success = (String) session.getAttribute("timerSet");
        if (success != null) {
            if (success.equals("successfull")) {
    %><h4>
        Timer was set Successfully
    </h4>><%
                session.setAttribute("timerSet", "notSet");
            }
        }
    %>
